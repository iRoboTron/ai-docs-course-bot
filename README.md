...
## Как открыть код конкретного урока

Этот репозиторий не похож на Colab-ноутбуки части 1: там каждый урок —
отдельный самодостаточный файл, а здесь один сервис растёт от урока к уроку,
и код урока 2 **меняет** файлы, которые создал урок 1. В `main` виден только
самый свежий результат — версии «ровно как в конце урока 7.1» без правок
урока 7.2 в рабочей копии уже нет.

Чтобы посмотреть код именно на конец нужного урока — есть git-теги:

```bash
git tag -l                # список всех уроков с кодом
git checkout urok-7.1     # снимок кода ровно как в конце урока 7.1
cat app/rag.py            # RAG ещё без файлового кэша

git checkout urok-7.2     # урок 7.2 — тот же файл уже с storage.py
git checkout main         # вернуться к актуальному состоянию сервиса
```

`git checkout urok-...` переводит репозиторий в detached HEAD — это только
для чтения: посмотрели, вернулись на `main` командой выше. Коммитить в этом
состоянии не нужно.
